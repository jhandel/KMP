...
class BranchesTable extends BaseTable
{
    ...
    {
        $descendants = Cache::read(TenantContext::cacheKey('descendants_' . $id), 'branch_structure');
        if (!$descendants) {
            $descendants = $this->getDescendantsLookup();
            foreach ($descendants as $key => $value) {
                Cache::write(TenantContext::cacheKey('descendants_' . $key), $value, 'branch_structure');
            }
    ...
    {
        $parents = Cache::read(TenantContext::cacheKey('parents_' . $id), 'branch_structure');
        if (!$parents) {
            $parents = $this->getParentsLookup();
            foreach ($parents as $key => $value) {
                Cache::write(TenantContext::cacheKey('parents_' . $key), $value, 'branch_structure');
            }
            $parents = $parents[$id] ?? [];
        }
