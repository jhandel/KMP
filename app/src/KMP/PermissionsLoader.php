...
class PermissionsLoader
{
    ...
    public static function getPermissions(int $memberId): array
    {
        ...
        $cacheKey = TenantContext::cacheKey('member_permissions' . $memberId);
    ...
    public static function getPolicies($id, ?array $branchIds = null): array
    {
        ...
        $cacheKey = TenantContext::cacheKey('permissions_policies' . $id);
        $cache = Cache::read($cacheKey, 'member_permissions');
        if ($cache) {
            return $cache; // Return cached result if available
        }
    ...
    public static function getMembersWithPermissionsQuery(int $permissionId, int $branch_id): SelectQuery
    {
        ...
        $subquery = $permissionsTable
            ->find()
            ->cache(TenantContext::cacheKey('permissions_members' . $permissionId), 'permissions_structure');
    ...
    {
        // 1. Cache Strategy - Check for cached permissions first
        $cacheKey = TenantContext::cacheKey('sp_permissions_' . $servicePrincipalId);
        $cache = Cache::read($cacheKey, 'member_permissions');
        if ($cache) {
            return $cache;
        }
    ...
    {
        // 1. Cache Strategy
        $cacheKey = TenantContext::cacheKey('sp_policies_' . $servicePrincipalId);
        $cache = Cache::read($cacheKey, 'member_permissions');
        if ($cache) {
            return $cache;
        }
