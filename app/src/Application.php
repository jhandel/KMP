    ...
    public function middleware(
        ...
        $middlewareQueue
            ...
            ->add(function ($request, $handler) {
                ...
                $updaterAllowedHosts = Configure::read('Updater.allowedHosts', []);
                if (is_array($updaterAllowedHosts)) {
                    foreach ($updaterAllowedHosts as $allowedHost) {
                        if (!is_string($allowedHost)) {
                            continue;
                        }
                        $normalizedHost = strtolower(trim($allowedHost));
                        if ($normalizedHost === '') {
                            continue;
                        }
                        $connectSources[] = 'https://' . $normalizedHost;
                    }
                }
                ...
                $csp = "default-src 'self'; " .                                              // Default to same origin
                    ...
                    "connect-src " . implode(' ', $connectSources) . "; " .           // AJAX/fetch restrictions + updater hosts
