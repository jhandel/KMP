...
`platform_admin:seed` creates the account only if missing unless `--force` is passed. Seeded and reset accounts must change their password on next browser login unless `--no-require-change` is used. Both commands print the initial or reset password; login and privileged-action verification codes are emailed to the platform admin address.
