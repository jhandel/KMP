...
Environment variables control deployment-specific configuration without modifying code. They can be supplied by process environment variables, Docker Compose, or `.env` files depending on the workflow, and support multiple deployment environments (development, staging, production) with a single codebase.
...
- **Document Storage**: Cloud storage credentials (if using Azure or S3)
...
For non-Docker local development and many deployment patterns, CakePHP loads environment variables from `.env` files in the `config/` directory:
...
1. Existing process/container environment variables
2. `config/.env` file - Loaded via `josegonzalez/dotenv` only when `APP_NAME` is not already set
3. `config/app.php` - Baseline configuration (uses `env()` calls to read environment values)
4. `config/app_local.php` - Environment-specific overrides (if present)
5. `config/app_queue.php` - Queue plugin configuration (if present)

Later values override earlier ones. Environment variables set in `.env` are consumed by `env()` calls in `app.php`.

### Docker Compose Agentic Development

The local Docker Compose workflow used for host-based/agentic development does not use `app/config/.env` for application runtime configuration.

Instead:

1. Docker Compose reads shell variables and an optional repo-root `.env` file for substitution in `docker-compose.yml`.
2. `docker-compose.yml` passes the resolved values into the `app`, `db`, and `mailpit` containers through each service's `environment:` block.
3. The app container sets `APP_NAME=KMP_DOCKER`, so `config/bootstrap.php` skips CakePHP dotenv loading.
4. `docker/entrypoint.sh` copies `docker/app_local.php` into `app/config/app_local.php`.
5. `docker/app_local.php` reads container environment variables such as `MYSQL_HOST`, `PLATFORM_DB_HOST`, `TENANT_DB_HOST`, and `EMAIL_SMTP_HOST`.

Use a repo-root `.env` file for Docker Compose overrides:

```bash
cp docker/.env.example .env
```

Do not put Docker Compose development secrets in `app/config/.env`; that file is intentionally bypassed in the compose workflow.
...
- `AWS_SESSION_TOKEN` - Temporary S3 credentials token
