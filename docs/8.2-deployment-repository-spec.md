...
If an update process crashes and leaves `apply.lock`, KMP treats that lock as stale after `UPDATER_LOCK_TIMEOUT_SECONDS` (default `1800`) and will automatically replace it.
...
Current sync scope is limited to runtime application paths inside `app/` (`bin`, `config`, `plugins`, `resources`, `src`, `templates`, `webroot`, plus `index.php`/`.htaccess`) so deployments with read-only dev/build files (for example `app/psalm.xml`) and root-owned dependency trees can still update safely from the web UI.
