...
Current sync scope is limited to the package `app/` tree (runtime application content) so deployments with read-only repository root files can still update safely from the web UI.
