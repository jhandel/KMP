...
Current sync scope is limited to runtime application paths inside `app/` (`bin`, `config`, `plugins`, `resources`, `src`, `templates`, `vendor`, `webroot`, plus `index.php`/`.htaccess`) so deployments with read-only dev/build files (for example `app/psalm.xml`) can still update safely from the web UI.
