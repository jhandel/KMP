...
- Re-evaluates all entities extending `ActiveWindowBaseEntity` (e.g., `Warrants`) and transitions:
  ...
  - `Current → Expired` when `expires_on` is populated and in the past
...
## 🔄 Workflow-Managed Tables

Tables with an active workflow definition (in `workflow_definitions` where `is_active = true`) are automatically **skipped** by `sync_active_window_statuses`. These tables are instead handled by the `sync_workflow_temporal_statuses` command, which fires transitions through the workflow engine so that `on_enter_actions` execute correctly.

When a workflow-managed table is skipped, the command logs: `Skipping {tableName}: managed by workflow engine`.

### Running `sync_workflow_temporal_statuses`

```bash
# Dry run
bash cronScripts/runCakeCommand.sh \
  --workdir /home/vscribe/amp-uat.ansteorra.org \
  sync_workflow_temporal_statuses --dry-run

# Execute
bash cronScripts/runCakeCommand.sh \
  --workdir /home/vscribe/amp-prod.ansteorra.org \
  sync_workflow_temporal_statuses
```

### Cron: AMP UAT (every 5 minutes, dry-run)

```
*/5 * * * * /usr/bin/setlock -n /tmp/cronlock.vscribe-AMP_UAT_SyncWorkflow \
  sh -c 'bash /home/vscribe/cronJobScripts/runCakeCommand.sh \
    --workdir /home/vscribe/amp-uat.ansteorra.org \
    sync_workflow_temporal_statuses --dry-run'
```

### Cron: AMP PROD (nightly at 01:35)

```
35 1 * * * /usr/bin/setlock -n /tmp/cronlock.vscribe-AMP_PROD_SyncWorkflow \
  sh -c 'bash /home/vscribe/cronJobScripts/runCakeCommand.sh \
    --workdir /home/vscribe/amp-prod.ansteorra.org \
    sync_workflow_temporal_statuses'
```
...
- `app/src/Command/SyncWorkflowTemporalStatusesCommand.php`
