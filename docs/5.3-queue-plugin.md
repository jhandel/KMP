...
### Multi-Tenant Queue Operation

In managed multi-tenant deployments, queue tables live in each tenant database rather than the platform registry. Jobs enqueued under an active tenant context carry tenant metadata in the job payload. Workers restore or verify that tenant context before executing the task.

Run workers with an explicit tenant:

```bash
bin/cake queue run --tenant=example
```

Use one worker or supervised process per active tenant, or a controlled `--all-tenants` loop for scheduled maintenance. Scheduled runners should usually include `--exit-when-empty` so each tenant is checked and the command exits after all currently ready jobs are drained:

```bash
bin/cake queue run --all-tenants --exit-when-empty --max-runtime 270 -q
```

A worker connected to one tenant must not process another tenant's jobs. In container deployments, queue runners must pass `--tenant=<slug>` or `--all-tenants`; running `queue run` without tenant selection only watches the ambient/default datasource and will not drain queues stored in separate tenant databases.
...
```bash
# Start a worker process for one tenant
bin/cake queue run --tenant=example

# Scheduled multi-tenant pass
bin/cake queue run --all-tenants --exit-when-empty --max-runtime 270 -q

# Run worker with specific groups
bin/cake queue run --tenant=example -g notifications,reports

# Run worker with specific task types
bin/cake queue run --tenant=example -t Email,Mailer

# Exclude specific task types
bin/cake queue run --tenant=example -t "-ExampleTask"

# Verbose output
bin/cake queue run --tenant=example -v
```
...
command=/path/to/kmp/bin/cake queue run --tenant=example
