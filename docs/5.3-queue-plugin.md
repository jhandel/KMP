...
Use one worker or supervised process per active tenant, or a controlled `--all-tenants` loop for scheduled maintenance. A worker connected to one tenant must not process another tenant's jobs.
